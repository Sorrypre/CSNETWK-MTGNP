...
class PriorityGrant(BaseModel):
    type: Literal[PDUType.PRIORITY_GRANT]
    seq_num: int
    player_id: str = Field(min_length=1)
    time_limit_ms: int

class PriorityPass(BaseModel):
    type: Literal[PDUType.PRIORITY_PASS]
    seq_num: int

class CastSpell(BaseModel):
    type: Literal[PDUType.CAST_SPELL]
    seq_num: int
    card_id: str
    targets: List[str]
...
class ActivateAbility(BaseModel):
    type: Literal[PDUType.ACTIVATE_ABILITY]
    seq_num: int
    source_id: str
    ability_index: int
    targets: List[str]
...
class StackPush(BaseModel):
    type: Literal[PDUType.STACK_PUSH]
    seq_num: int
    stack_item_id: str
...
class TriggerOrder(BaseModel):
    type: Literal[PDUType.TRIGGER_ORDER]
    seq_num: int
    player_id: str
    trigger_ids: List[str]

class TriggerOrderResponse(BaseModel):
    type: Literal[PDUType.TRIGGER_ORDER_RESPONSE]
    seq_num: int
    ordered_trigger_ids: List[str]

class TriggerChoice(BaseModel):
    type: Literal[PDUType.TRIGGER_CHOICE]
    seq_num: int
    trigger_id: str
    source_id: str
    effect_summary: str
...
class TriggerChoiceResponse(BaseModel):
    type: Literal[PDUType.TRIGGER_CHOICE_RESPONSE]
    seq_num: int
    trigger_id: str
    accept: bool
...
class StackResolve(BaseModel):
    type: Literal[PDUType.STACK_RESOLVE]
    seq_num: int
    stack_item_id: str
...
class Ping(BaseModel):
    type: Literal[PDUType.PING]
    seq_num: int
    timestamp: int

class Pong(BaseModel):
    type: Literal[PDUType.PONG]
    seq_num: int
    timestamp: int